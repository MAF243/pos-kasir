            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-3xl">
                <form onSubmit={submit}>
                    <div className="p-6 space-y-6">
                        <div>
                            ...
                            <input
                                ...
                                disabled={role.name === 'Admin'} // Protect Admin name change
                            />
                            {errors.name && <p className="mt-2 text-sm text-red-600">{errors.name}</p>}
                            {role.name === 'Admin' && <p className="mt-1 text-xs text-gray-500">The Admin role name cannot be changed.</p>}
                        </div>
                                    ...
                                    <div key={permission.id} className="flex items-start">
                                        <div className="flex items-center h-5">
                                            <input
                                                ...
                                                disabled={role.name === 'Admin'} // Admin automatically has all permissions
                                                className="focus:ring-primary-500 h-4 w-4 text-primary-600 border-gray-300 rounded disabled:opacity-50"
                                            />
                                        </div>
                                        <div className="ml-3 text-sm">
                                            <label htmlFor={`permission-${permission.id}`} className={`font-medium ${role.name === 'Admin' ? 'text-gray-400' : 'text-gray-700'}`}>
                                                {permission.name}
                                            </label>
                                        </div>
                                    </div>
                    ...
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
                        ...
                        <button
                            type="submit"
                            disabled={processing || role.name === 'Admin'} // Prevent updating Admin role permissions via UI
