...
export default function Create({ suppliers, products }) {
    ...
    const grandTotal = useMemo(() => {
        return data.items.reduce((total, item) => total + (Number(item.qty) * Number(item.purchase_price)), 0);
    }, [data.items]);
    ...
    const submit = (e) => {
        ...
        const hasPricingError = data.items.some(item => Number(item.purchase_price) > Number(item.selling_price));
        if (hasPricingError) {
            alert('Cannot proceed: One or more products have a purchase price higher than their selling price. You will incur a loss!');
            return;
        }
    ...
    return (
        <AdminLayout title="New Supplier Purchase">
                ...
                <form onSubmit={submit}>
                    ...
                    <div className="p-6 bg-gray-50 border-b border-gray-100 flex items-end gap-4">
                        <div className="flex-1">
                            <label className="block text-sm font-medium text-gray-700">Select Product to Add</label>
                            <select
                            ...
                            >
                                <option value="" disabled>Search and select product...</option>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">Qty</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-1/4">Purchase Price (IDR)</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {data.items.length === 0 ? (
                                    ...
                                    data.items.map((item, index) => {
                                        const isLoss = Number(item.purchase_price) > Number(item.selling_price);
                                        return (
                                        <tr key={item.product_id} className={isLoss ? 'bg-red-50' : ''}>
                                            <td className="px-6 py-4">
                                                <div className="text-sm font-medium text-gray-900">{item.name}</div>
                                                <div className="text-xs text-gray-500">Selling Price: {formatCurrency(item.selling_price)}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <input
                                                    type="number"
                                            ...
                                            <td className="px-6 py-4">
                                                <input
                                                    ...
                                                    required
                                                    min="0"
                                                    className={`block w-full text-right px-3 py-2 border rounded-md shadow-sm sm:text-sm ${isLoss ? 'border-red-500 focus:ring-red-500 text-red-700 bg-red-50' : 'border-gray-300 focus:ring-primary-500 focus:border-primary-500'}`}
                                                ...
                                                {isLoss && (
                                                    <p className="mt-1 text-xs text-red-600 flex items-center">
                                                        ...
                                                        Warning: Exceeds Selling Price!
                                                    </p>
                                                )}
                                                {errors[`items.${index}.purchase_price`] && <p className="mt-1 text-xs text-red-600">{errors[`items.${index}.purchase_price`]}</p>}
                                            </td>
                                            <td className="px-6 py-4 text-right whitespace-nowrap text-sm font-medium text-gray-900">
                                                {formatCurrency(Number(item.qty) * Number(item.purchase_price))}
                                            </td>
                            ...
                            <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                                <tr>
                                    ...
                                    <td className="px-6 py-4 text-right text-xl font-bold text-primary-700">{formatCurrency(grandTotal)}</td>
                                    <td></td>
                                </tr>
                            </tfoot>
                        </table>
                    ...
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-end gap-3">
                        <Link href="/purchases" className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
                            Cancel
                        </Link>
                        <button
                            type="submit"
